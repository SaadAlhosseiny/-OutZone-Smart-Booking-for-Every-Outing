      ...
      <div className="sticky top-0 z-20 bg-white/95 dark:bg-[#111]/95 backdrop-blur-sm border-b border-stone-100 dark:border-white/5 px-6 py-4">
        <div className="max-w-screen-xl mx-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <Icons.Heart size={20} className="text-red-400" />
            <h1 className="text-xl font-bold text-stone-900 dark:text-white">Saved Places</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8">
        {!isLoggedIn ? (
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
            ...
            <div className="h-20 w-20 rounded-full bg-stone-100 dark:bg-white/5 flex items-center justify-center mb-4">
              <Icons.Heart size={36} className="text-stone-300 dark:text-white/20" />
            </div>
            <h2 className="text-xl font-bold text-stone-700 dark:text-white mb-2">Sign in to see your saved places</h2>
            <p className="text-stone-400 dark:text-white/30 text-sm mb-6">Save your favorite venues and find them here</p>
            <button onClick={() => navigate('/')}
              className="px-6 py-3 rounded-xl bg-[#f9a825] text-white font-semibold text-sm hover:shadow-md transition-all">
              Go to Home
            </button>
          </motion.div>
        ) : loading ? (
          ...
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
            ...
            <p className="text-stone-400 dark:text-white/30 text-sm mb-6">Tap the heart on any venue to save it here</p>
            <button onClick={() => navigate('/')}
              className="px-6 py-3 rounded-xl bg-[#f9a825] text-white font-semibold text-sm hover:shadow-md transition-all">
              Discover Places
            </button>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              <AnimatePresence>
                {favorites.map((fav, i) => (
                  <motion.div key={fav.id} initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
                    ...
                    <div className="relative h-44 overflow-hidden cursor-pointer" onClick={() => navigate(`/place/${fav.place?.id}`)}>
                      <img src={fav.place?.image_url} alt={fav.place?.name}
                        onError={e => { e.target.src='https://placehold.co/400x300/e2e8f0/64748b?text=Venue'; }}
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      <button onClick={e => { e.stopPropagation(); handleRemove(fav.place?.id); }}
                        className="absolute top-2 right-2 p-2 rounded-full bg-white/90 dark:bg-black/50 backdrop-blur-sm shadow-md hover:bg-red-50 transition-colors">
                    ...
                    <div className="p-4 cursor-pointer" onClick={() => navigate(`/place/${fav.place?.id}`)}>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs font-semibold text-stone-600 dark:text-white/60">{fav.place?.rating}</span>
                        </div>
