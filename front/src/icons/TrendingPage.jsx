...
function mapPlace(p) {
  return { id: p.id, name: p.name, type: p.category, rating: p.rating,
    ...
    location: p.location, audience: p.audience };
}
...
export default function TrendingPage() {
  ...
  useEffect(() => {
    // الأكثر تقييماً = الأكثر رواجاً
    api.getPlaces().then(d => {
      const sorted = (d||[]).map(mapPlace).sort((a, b) => b.rating - a.rating);
      setVenues(sorted);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#111] transition-colors duration-300">
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8">
        <p className="text-sm text-stone-400 dark:text-white/30 mb-6">Top rated places in Cairo</p>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {venues.map((venue, i) => (
              <motion.div key={venue.id} initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay: i*0.04 }}
                ...
                {/* Rank */}
                <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${i === 0 ? 'bg-amber-400 text-white' : i === 1 ? 'bg-stone-300 dark:bg-white/20 text-stone-700 dark:text-white' : i === 2 ? 'bg-amber-700/20 text-amber-700 dark:text-amber-400' : 'bg-stone-100 dark:bg-white/5 text-stone-400 dark:text-white/30'}`}>
                  {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `#${i+1}`}
                </div>
                {/* Image */}
                <div className="shrink-0 h-16 w-16 rounded-xl overflow-hidden">
                  <img src={venue.images[0]} alt={venue.name} onError={e => { e.target.src='https://placehold.co/100x100/e2e8f0/64748b?text=V'; }}
                    className="w-full h-full object-cover" />
                </div>
                {/* Info */}
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-stone-900 dark:text-white text-sm line-clamp-1">{venue.name}</h3>
                  <p className="text-xs text-stone-400 dark:text-white/30 capitalize">{venue.type} · {venue.location?.split(',')[0]}</p>
                  <div className="flex items-center gap-3 mt-1">
                    <div className="flex items-center gap-1">
                      <Icons.Star size={11} className="text-amber-400 fill-amber-400" />
                      <span className="text-xs font-bold text-stone-700 dark:text-white">{venue.rating}</span>
                    </div>
