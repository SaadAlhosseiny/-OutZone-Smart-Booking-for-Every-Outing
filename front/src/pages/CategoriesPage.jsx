...
const CATEGORY_ICONS = {
  ...
  'art spaces': 'Palette', 'cultural centers': 'Music2', 'theme parks': 'Star',
  'live music': 'Music', 'arcade': 'Gamepad2', 'family entertainment': 'Users',
};

function mapPlace(p) {
  return { id: p.id, name: p.name, type: p.category, rating: p.rating,
    images: [p.image_url], price_range: p.price_range, location: p.location };
}

function VenueCard({ venue, onClick }) {
  return (
    <div onClick={onClick} className="bg-white dark:bg-[#1e1e1e] rounded-xl overflow-hidden border border-stone-100 dark:border-white/5 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer">
      <div className="h-40 overflow-hidden">
        <img src={venue.images[0]} alt={venue.name} onError={e => { e.target.src='https://placehold.co/400x300/e2e8f0/64748b?text=Venue'; }} className="w-full h-full object-cover" />
      </div>
      <div className="p-3">
        <h3 className="font-bold text-stone-900 dark:text-white text-sm line-clamp-1">{venue.name}</h3>
        <div className="flex items-center justify-between mt-1">
          <div className="flex items-center gap-1"><Icons.Star size={11} className="text-amber-400 fill-amber-400" /><span className="text-xs font-semibold text-stone-600 dark:text-white/60">{venue.rating}</span></div>
...
export default function CategoriesPage() {
  ...
  const filtered = activeCategory ? allVenues.filter(v => v.type === activeCategory) : [];
    ...
    <div className="min-h-screen bg-white dark:bg-[#111] transition-colors duration-300">
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8">
        <div className="flex flex-wrap gap-2 mb-8">
          <button onClick={() => setActiveCategory(null)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all border ${!activeCategory ? 'bg-[#f9a825] border-[#f9a825] text-white' : 'bg-white dark:bg-white/5 border-stone-200 dark:border-white/10 text-stone-600 dark:text-white/60 hover:border-[#f9a825]'}`}>All</button>
          {categories.map(cat => {
            const Icon = Icons[CATEGORY_ICONS[cat] || 'MapPin'] || Icons['MapPin'];
            return (
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filtered.map(venue => <VenueCard key={venue.id} venue={venue} onClick={() => navigate(`/place/${venue.id}`)} />)}
          </motion.div>
        ) : (
              ...
              return (
                <motion.div key={cat} initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setActiveCategory(cat)} className="text-sm text-[#f9a825] hover:underline flex items-center gap-1">See all <Icons.ChevronRight size={14} /></button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {catVenues.slice(0,4).map(venue => <VenueCard key={venue.id} venue={venue} onClick={() => navigate(`/place/${venue.id}`)} />)}
                  </div>
                </motion.div>
              );
            })}
