...
function mapPlace(p) {
  return { id: p.id, name: p.name, type: p.category, rating: p.rating,
    description: p.description, images: [p.image_url], isTrending: p.rating >= 4.7,
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      if (!search.trim()) { api.getPlaces().then(d => setVenues((d||[]).map(mapPlace))); return; }
      setLoading(true);
      try {
        const result = await api.recommend({ q: search.trim(), top_n: 20 });
        const recs = result?.recommendations || [];
        if (recs.length > 0) setVenues(recs.map(r => mapPlace(r.place)));
        else { const data = await api.getPlaces({ q: search.trim() }); setVenues((data||[]).map(mapPlace)); }
      } catch { const data = await api.getPlaces({ q: search.trim() }).catch(() => []); setVenues((data||[]).map(mapPlace)); }
              ...
              {venues.map((venue, i) => (
                <motion.div key={venue.id} initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay: i*0.03 }}
                  ...
                  <div className="relative h-44 overflow-hidden">
                    ...
                    {venue.isTrending && <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-full bg-[#f9a825] text-white text-xs font-semibold"><Icons.TrendingUp size={10} /> Trending</div>}
                  </div>
                  <div className="p-4">
