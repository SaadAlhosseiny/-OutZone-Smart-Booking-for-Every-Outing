...
export default function VenueCardGrid({ venues = [] }) {
    ...
    return (
      <section className="w-full bg-white dark:bg-[#111] min-h-[40vh] flex items-center justify-center transition-colors duration-300">
        <div className="text-center py-20">
          ...
          <h3 className={`text-xl font-semibold text-stone-700 dark:text-white/50 mb-2 ${t.fontClass}`}>{t.noVenues}</h3>
          <p className={`text-stone-500 dark:text-white/30 ${t.fontClass}`}>{t.noVenuesSub}</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="w-full bg-white dark:bg-[#111] min-h-screen transition-colors duration-300" dir={t.dir}>
      <div className="max-w-screen-xl mx-auto px-6 md:px-8 py-10 md:py-14">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-xs font-semibold tracking-widest uppercase text-amber-600 dark:text-amber-400 ${t.fontClass}`}>
              {t.curatedForYou}
            </span>
          </div>
          <h2 className={`text-3xl md:text-4xl font-bold text-stone-900 dark:text-white tracking-tight ${t.fontClass}`}>
            {t.cairoFinest}
          </h2>
          <p className={`mt-2 text-stone-500 dark:text-white/40 text-base max-w-2xl ${t.fontClass}`}>
            {venues.length} {venues.length !== 1 ? t.venuesFound : t.venueFound}
          </p>
        </motion.div>
            ...
            >
              <div className="bg-white dark:bg-[#1e1e1e] rounded-xl overflow-hidden shadow-sm hover:shadow-xl dark:shadow-black/40 transition-all duration-500 border border-stone-100 dark:border-white/5 hover:-translate-y-1.5">
                <div className="relative h-48 overflow-hidden">
                  ...
                  <button onClick={e => toggleFavorite(e, venue.id)}
                ...
                <div className="px-4 pb-4 pt-3">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs text-stone-400 dark:text-white/30 ${t.fontClass}`}>{t.budgetLabel}</span>
                      {renderBudget(venue.budget)}
                      {venue.price_range && (
                        <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 ml-1">{venue.price_range}</span>
                      )}
                    </div>
                  ...
                  <div className={`flex items-center gap-1 text-xs font-semibold transition-all duration-300 ${t.fontClass} ${hoveredCard === venue.id ? 'text-amber-600 dark:text-[#f9a825] translate-x-1' : 'text-stone-400 dark:text-white/20'}`}>
                    {t.viewDetails}
